...
include("08_participation_ratio_analysis.jl")
...
function run_all_lpca_publication_experiments()
    ...
    println("\n>>> [STEP 1/8] Running K-Dependency Analysis...")
    run_k_dependency_experiment()

    println("\n>>> [STEP 2/8] Running Data Normalization Comparisons...")
    run_normalization_experiment()

    println("\n>>> [STEP 3/8] Running Coordinate & Scale Invariance Tests...")
    run_coordinate_invariance_experiment()

    println("\n>>> [STEP 4/8] Running Dimension Statistics & dims() Evaluation...")
    run_dimension_statistics_experiment()

    println("\n>>> [STEP 5/8] Generating 2D & 3D Phase Space Evolution Grids...")
    run_phase_space_grids_experiment()

    println("\n>>> [STEP 6/8] Running Parameterization Focus Analysis (Dense K Sweep)...")
    run_parameterization_focus_experiment()

    println("\n>>> [STEP 7/8] Running Soft-Weighted LPCA (Sigmoidal & Density-Weighted)...")
    run_soft_weighted_lpca_experiment()

    println("\n>>> [STEP 8/8] Running Participation Ratio (PR) Dimension Analysis...")
